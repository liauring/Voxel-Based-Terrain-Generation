...
void CreateGIF(const char *gifFilename, int frameCount);
...
int main() {
    ...
    for (int i = 0; i < 1; i++) {
        ...
        DrawFrontToBack((CustomPoint){(float)670, (float)(500 - i * 16)}, 0, 120, 1000, (CustomPoint){(float)670, (float)(500 - i * 16)});
...
void DrawFrontToBack(CustomPoint p, float phi, float height, float distance, CustomPoint pmap) {
    ...
    for (float z = 5; z < distance; z += dz) {
        ...
        if(itr % 100 == 0){
            SaveFrameAsImage(framecnt); // Save the current frame as an image
            framecnt++;
        }
        itr++;
    }
