...
int main() {
    ...
    uint32_t* d_screens[MAX_FRAMES];  // Array to hold device screen pointers
    uint8_t* d_heightmap;
    uint32_t* d_colormap;

    initCudaMemory(d_screens, &d_heightmap, &d_colormap);
    copyDataToGPU(d_heightmap, heightmap, d_colormap, colormap);
    
    const auto compute_start = std::chrono::steady_clock::now();
    
    // Set parameters for rendering
    float p_x = 670.0f;  // Example camera position
    float p_y = 500.0f;  // Example camera position
    float phi = 0.0f;    // Example rotation angle
    float height = 120.0f; // Example height
    float distance = 10000.0f; // Example distance
    int kernel_frames = 5;  // Number of frames to render in one kernel call

    // loop over max frames
    for (int i = 0; i < num_loop; i++) {
        for (int j = 0; j < kernel_frames; j++) {
            cudaCheckError(cudaMalloc(&d_screens[j], WIDTH * HEIGHT * sizeof(uint32_t)));
        }

        // Launch the rendering kernel for all frames
        launchRenderKernel(
            d_screens,
            d_heightmap,
            d_colormap,
            p_x,
            p_y - i * 16 * kernel_frames,
            phi,
            height,
            distance,
            kernel_frames
        ...
        // Copy the rendered frames back to host memory and save images
        for (int j = 0; j < kernel_frames; j++) {
            cudaCheckError(cudaMemcpy(screen, d_screens[j], WIDTH * HEIGHT * sizeof(uint32_t), cudaMemcpyDeviceToHost));
            printf("Saving frame %d\n", j + i * kernel_frames);
            SaveFrameAsImage(j + i * kernel_frames);
            cudaFree(d_screens[j]);  // Free device memory for this screen
        }
    }
    ...
    freeCudaMemory(d_heightmap, d_colormap);
